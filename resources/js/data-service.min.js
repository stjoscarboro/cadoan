...
special:a[4]})})}));a.all(h).then(function(){b.resolve(g)});return b.promise},loadSingers:function(){var g=a.defer(),b=[];c.getSheetData("cadoan.singers").then(function(a){(a=a.data.values)&&a.forEach(function(a){a[1]&&b.push({id:a[0],name:a[1]})});g.resolve(b)},function(a){console.log(a.data.error)});return g.promise},loadSchedules:function(g){var b=a.defer(),h=[];c.getSheetData("cadoan.schedules").then(function(a){(a=a.data.values)&&a.forEach(function(a){var b=m(a[0]),c=JSON.parse(a[1]);a=JSON.parse(a[2]);
