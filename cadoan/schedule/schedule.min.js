...
Object.assign(c.liturgy,m.pick(h,"name","year"));a.schedules.push(c)}m.resizeFrame(a)})};a.sort=function(){k.updateSheetData("cadoan.schedules",{requests:[{sortRange:{range:{sheetId:0},sortSpecs:[{dimensionIndex:0,sortOrder:"ASCENDING"}]}}]}).then(function(){a.clear();a.get()})};a.clear=function(){a.lists={};a.schedule={liturgy:{},songs:[]};a.rows=5};a.create=function(){var e=f.open({scope:a,templateUrl:"editor.html",backdrop:"static",backdropClass:"light",keyboard:!1,controller:function(){a.submit=
function(){a.save().then(function(){e.close()})};a.cancel=function(){a.clear();e.close()}}});e.opened.then(function(){g(function(){var e=function(){for(var e=$.datepicker.parseDate(a.dateFormat,a.schedule.date),c=$jscomp.makeIterator(a.liturgies),b=c.next();!b.done;b=c.next())b=b.value,e.getTime()===b.date.getTime()&&(Object.assign(a.schedule.liturgy,m.pick(b,"id","name","year")),a.schedule.liturgy.special||(a.schedule.liturgy.special=b.special))};$("#datepicker").datepicker({dateFormat:a.dateFormat,
onSelect:function(b){a.schedule.date=b;a.schedule.liturgy={};e()}});if(!a.schedule.date)for(var b=$jscomp.makeIterator(a.schedules),c=b.next();!c.done;c=b.next()){c=$.datepicker.parseDate(a.dateFormat,c.value.date);var d=a.weekms-c.getDay()*a.dayms;a.schedule.date=$.datepicker.formatDate(a.dateFormat,new Date(c.getTime()+d))}e();(function(){if(0===a.schedule.songs.length)for(var b=0;5>b;b++)a.schedule.songs.push({category:a.categories[b]}),a.selectSongs(b)})();m.scrollFrame()},100)})};a.edit=function(b){a.schedule=
angular.copy(a.schedules[b]);a.rows=a.schedule.songs.length;(b=a.liturgies.find(function(b){return b.id===a.schedule.liturgy.id}))&&Object.assign(a.schedule.liturgy,m.pick(b,"name"));a.schedule.songs.forEach(function(b,e){var c=a.singers.find(function(a){return a.name===b.singer});c&&(b.singer=c.id);Object.assign(a.schedule.songs[e],m.pick(b,"id","category"));a.selectSongs(e)});a.create()};a.remove=function(e){var d=b.defer();k.updateSheetData("cadoan.schedules",{requests:[{deleteDimension:{range:{sheetId:0,
dimension:"ROWS",startIndex:e,endIndex:e+1}}}]}).then(function(){a.schedules.splice(e,1);d.resolve()});return d.promise};a.save=function(){for(var e=$.datepicker.parseDate(a.dateFormat,a.schedule.date),d={},f=[],c,g=[],h=b.defer(),p=$jscomp.makeIterator(a.schedule.songs),l=p.next();!l.done;l=p.next())f.push(Object.assign({},m.pick(l.value,"id","singer")));p=$jscomp.makeIterator(a.liturgies);for(l=p.next();!l.done;l=p.next())l=l.value,l.name===a.schedule.liturgy.name&&(a.schedule.liturgy.id=l.id);
a.schedule.liturgy.id?Object.assign(d,m.pick(a.schedule.liturgy,"id","year","special")):Object.assign(d,m.pick(a.schedule.liturgy,"name","year","special"));c={values:[[$.datepicker.formatDate("yy-mm-dd",e),JSON.stringify(d),JSON.stringify(f)]]};a.schedules.find(function(b,c){$.datepicker.parseDate(a.dateFormat,b.date).getTime()===e.getTime()&&g.push(a.remove(c))});b.all(g).then(function(){k.appendSheetData("cadoan.schedules",c,{valueInputOption:"USER_ENTERED"}).then(function(){a.sort();h.resolve()},
function(a){console.log(a.data.error);h.reject()})});return h.promise};a.loadData=function(){var e=b.defer();b.all([r.listFolder("cadoan.sheets"),k.loadLiturgies(),k.loadSingers()]).then(function(b){for(var d=$jscomp.makeIterator(b[0]),c=d.next();!c.done;c=d.next())Array.prototype.push.apply(a.songs,c.value);a.categories=k.listCategories(a.songs);a.liturgies=b[1];a.singers=b[2];k.sortByLocale(a.songs,"title");k.sortByLocale(a.singers,"name");k.sortCategories(a.categories);e.resolve()});return e.promise};
a.selectSongs=function(b){var e=a.schedule.songs[b];if(e&&e.category){a.lists[b]={songs:[]};for(var d=$jscomp.makeIterator(a.songs),c=d.next();!c.done;c=d.next())c=c.value,c.category===e.category&&a.lists[b].songs.push(c)}};a.previewSong=function(b){(b=a.schedule.songs[b])&&b.id&&d.open(r.getOpenURL(b.id),"_blank")}}]);
